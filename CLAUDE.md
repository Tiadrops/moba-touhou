...
3. **レーザー (laser)**: `img/bullets/reazer.png`, 550x1100px, 横幅2m表示
...
| GROUP_B | 500 | 0 | 100 | 無制限 | B1(オーブ弾), B2(レーザー:横幅2m), B3(ラプチャー/スクリーム), B4(固定射撃/恐怖弾) |
| GROUP_C | 1500 | 0 | 100 | 無制限 | C1(Kaiser型: オブリテレイト/デスグラスプ), C2(Hisui型: W2/E/R) フラグ持ち（撃破でWaveクリア）|

#### 雑魚敵の表示仕様
- **HPバー**: 全雑魚の頭上に表示（プレイエリア内に入ると表示開始）
- **被弾Tint**: なし（元の画像色をそのまま使用）
